...
const ofp_match Flow::get_exact_match() const
{
    //TODO change to ofl_match!
    ofp_match om;
    om.wildcards = ntohl(0);
    om.in_port = ntohl(in_port);
...
Flow::Flow(uint32_t in_port_, const Buffer& buffer, uint64_t cookie_)
    : in_port(in_port_),
      dl_vlan(), dl_vlan_pcp(0), dl_src(), dl_dst(), dl_type(0),
      nw_src(0), nw_dst(0), nw_proto(0), nw_tos(0),
      tp_src(0), tp_dst(0),
      mpls_label(0), mpls_tc(0), cookie(cookie_)
{
